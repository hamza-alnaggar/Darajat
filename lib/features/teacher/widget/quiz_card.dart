...
class _QuizCardState extends State<QuizCard> {
  ...
  Future<void> _handleQuestionSaved(QuestionSubModels newQuestion) async {
    ...
    final persisted = await _saveQuizToBackend(updatedQuiz);

    if (persisted != null && mounted) {
      setState(() {
        _currentQuiz = persisted;
      });
  ...
  Future<void> _handleQuestionUpdated(QuestionSubModels updatedQuestion) async {
    ...
    try {
      final persisted = await _saveQuizToBackend(_currentQuiz);

      if (persisted == null && mounted) {
        setState(() => _currentQuiz = previousQuiz);
      } else if (persisted != null && mounted) {
        setState(() => _currentQuiz = persisted);
        widget.onQuizUpdated(_currentQuiz);
      }
    } finally {
      if (mounted) setState(() => _isSaving = false);
  ...
  /// Save quiz to backend. Returns the persisted Quiz (with quizId set) on success, null on failure.
  Future<Quiz?> _saveQuizToBackend(Quiz quiz) async {
    try {
      ...
      // Create new quiz
        ...
        if (newQuizId > 0) {
          final persisted = quiz.copyWith(quizId: newQuizId);
          return persisted;
        } else {
          return null;
        }
        ...
        Container(
          ...
          child: ExpansionTile(
            ...
            leading: Container(
              ...
              child: Icon(Icons.quiz, color: Colors.orange, size: 20.r),
            ),
            title: Row(
              children: [
                Expanded(
